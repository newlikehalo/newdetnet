...
class CTPN_Model(nn.Module):
    def __init__(self):
        ...
        # self.rpn_2 = BasicConv(256, 256, 3, 1, 1, bn=False)
        self.brnn_2 = nn.LSTM(256, 128, bidirectional=True, batch_first=True)
        self.lstm_fc_2 = BasicConv(256, 512, 1, 1, relu=True, bn=False)
        self.rpn_class_2 = BasicConv(512, 10 * 2, 1, 1, relu=False, bn=False)
        self.rpn_regress_2 = BasicConv(512, 10 * 2, 1, 1, relu=False, bn=False)
    ...
    def forward(self, x):
        ...
        p3_x1 = p3_x.permute(0, 2, 3, 1).contiguous()  # channels last
        p3_b = p3_x1.size()  # batch_size, h, w, c
        p3_x1 = p3_x1.view(p3_b[0] * p3_b[1], p3_b[2], p3_b[3])  # torch.Size([66, 120, 256])
        p3_x2, _ = self.brnn_2(p3_x1)  # [66, 120, 256]
        p3_xsz = p3_x.size()
        p3_x3 = p3_x2.view(p3_xsz[0], p3_xsz[2], p3_xsz[3], 256)  # torch.Size([4, 20, 20, 256])
        p3_x3 = p3_x3.permute(0, 3, 1, 2).contiguous()  # channels first ([1, 66, 120, 256])

        p3_x3 = self.lstm_fc_2(p3_x3)
        p3_x = p3_x3

        p3_cls = self.rpn_class_2(p3_x)
        p3_regr = self.rpn_regress_2(p3_x)
        p3_cls = p3_cls.permute(0, 2, 3, 1).contiguous()
        p3_regr = p3_regr.permute(0, 2, 3, 1).contiguous()
        p3_cls = p3_cls.view(p3_cls.size(0), p3_cls.size(1) * p3_cls.size(2) * 10, 2)
        p3_regr = p3_regr.view(p3_regr.size(0), p3_regr.size(1) * p3_regr.size(2) * 10, 2)
        """p4"""
        #[1, 256, 66, 120]
        ...
        return cls, regr,p3_cls,p3_regr
