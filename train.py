...
def save_checkpoint(state, epoch, loss_cls, loss_regr, loss_cls_8, loss_regr_8,loss, ext='pth.tar'):
    check_path = os.path.join(config.checkpoints_dir,
                              f'ctpn_ep{epoch:02d}_'
                              f'{loss_cls:.4f}_{loss_regr:.4f}_{loss_cls_8:.4f}_{loss_regr_8:.4f}_{loss:.4f}.{ext}')
...
if __name__ == '__main__':
    ...
    for epoch in range(resume_epoch+1, epochs):
        ...
        for batch_i, (imgs, clss, regrs,clss_8,regrs_8) in enumerate(dataloader):
            imgs = imgs.to(device)
            clss = clss.to(device)
            regrs = regrs.to(device)
            clss_8 = clss_8.to(device)
            regrs_8 = regrs_8.to(device)
            
            optimizer.zero_grad()

            out_cls, out_regr,out_cls_8,out_regr_8 = model(imgs)  #torch.Size([1, 78000, 2])
            loss_cls = critetion_cls(out_cls, clss) #torch.Size([1, 1, 78000])
            loss_regr = critetion_regr(out_regr, regrs) #torch.Size([1, 78000, 3])

            loss_cls_8 = critetion_cls(out_cls_8, clss_8)  # torch.Size([1, 1, 78000])
            loss_regr_8 = critetion_regr(out_regr_8, regrs_8)  # torch.Size([1, 78000, 3])

            loss = loss_cls + loss_regr+loss_cls_8+loss_regr_8  # total loss
            loss.backward()
            optimizer.step()

            epoch_loss_cls += loss_cls.item()
            epoch_loss_regr += loss_regr.item()
            epoch_loss_cls_8 += loss_cls_8.item()
            epoch_loss_regr_8 += loss_regr_8.item()
            ...
            print(f'Ep:{epoch}/{epochs-1}--'
                  ...
                  f'batch: loss_cls_8:{loss_cls_8.item():.4f}--loss_regr_8:{loss_regr_8.item():.4f}--loss:{loss.item():.4f}\n'
                  f'Epoch: loss_cls_8:{epoch_loss_cls_8/mmp:.4f}--loss_regr_8:{epoch_loss_regr_8/mmp:.4f}--'
                  f'loss:{epoch_loss/mmp:.4f}\n')
        scheduler.step(epoch)
        epoch_loss_cls /= epoch_size
        epoch_loss_regr /= epoch_size
        epoch_loss_cls_8 /= epoch_size_8
        epoch_loss_regr_8 /= epoch_size_8
        epoch_loss /= epoch_size
        print(f'Epoch:{epoch}--{epoch_loss_cls:.4f}--{epoch_loss_regr:.4f}----{epoch_loss_cls_8:.4f}--{epoch_loss_regr_8:.4f}--{epoch_loss:.4f}')
        if epoch%1==0:
            best_loss = epoch_loss
            best_loss_regr = epoch_loss_regr
            best_loss_cls = epoch_loss_cls
            best_loss_regr_8 = epoch_loss_regr_8
            best_loss_cls_8 = epoch_loss_cls_8
            best_model = model
